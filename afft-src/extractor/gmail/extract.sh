...
ourPath=$(pwd)
ls -1 $1/mount/userdata/data/com.google.android.gm/databases | grep -e "mailstore" | grep -v "db-wal" | grep -v "db.shm" | while read -r accountdb;					#Finds the required databases
do
...
cp $1/mount/userdata/data/com.google.android.gm/databases/$accountdb $1/extracted\ data/gmail/$account/db/mailstore.db #Copies over the mailstore database for that account
