...
def makereports(case, timeline):
	...
	if os.path.exists(os.path.join(userdatadir, "data", "com.android.providers.contacts")):
		...
		if not os.path.exists(outdir):
			os.makedirs(outdir)
		report.call_log.maketable.makereport(case, timeline)
		print("--> Contacts\n")
	...
	if os.path.exists(os.path.join(userdatadir, "data", "com.android.providers.calendar")):
		...
		if not os.path.exists(outdir):
			os.makedirs(outdir)
		report.calendar.maketable.makereport(case, timeline)
...
def makeindex(case, timeline):
	...
	for item in os.listdir(datadir):
		if os.path.isdir(os.path.join(datadir, item)):
			...
			index.write(link)
	if timeline:
		link = "<TR><TD><a href=\"../timeline/report.html\"> Timeline </a> </TD></TR>\n"
		index.write(link)
